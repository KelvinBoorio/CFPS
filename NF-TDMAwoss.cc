      ...
      if(T_group-i+1 == 1) {
        ...
        for(int j=1; j<NumberOfNodes; j++) {
          ...
          if(AllNodesInformation[j].layer == 4 && flag[4] && 
            ...
            energy = ((*(node+j))->GetObject<ns3::energy::EnergySourceContainer> ()->Get (0)->GetEnergyFraction ()) * 100;
...
void UanExperiment::SaveResultsToCsv(const std::string &filename) {
    std::ofstream file(filename, std::ios::app); // 打开文件（追加模式）
    if (file.is_open()) {
        file << "RunNumber,T_total,Throughput\n"; // 表头
        file << 1 << "," << T_total << "," << Throughput << "\n"; // 数据
        file.close(); // 关闭文件
        NS_LOG_UNCOND("Results saved to " << filename);
    } else {
...
int main (int argc, char *argv[]) {
  ns3::LogComponentDisableAll(LOG_WARN); // 禁用警告日誌
  CommandLine cmd;
  cmd.Parse (argc, argv);
  ...
  // 儲存結果到 Excel
  experiment.SaveResultsToCsv("NF-TDMA-results.csv");
